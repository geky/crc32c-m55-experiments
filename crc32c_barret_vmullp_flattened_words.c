...
// vmull.p16 instruction, flattened and attempted hand-optimized, a word
// at a time
...
uint32_t crc32c_barret_vmullp_flattened_words(
    ...
    for (size_t i = 0; i < size;) {
        if (((uintptr_t)&data_[i]) % 4 == 0 && i+4 <= size) {
            ...
            a_ = __arm_vreinterpretq_u16_u32(
                    __arm_vsetq_lane_u32(a, __arm_vreinterpretq_u32_u16(a_), 3));
            uint16x8_t x_ = __arm_vreinterpretq_u16_u32(
                    __arm_vmulltq_poly_p16(a_, c_b_));

            // pmull(lo32(_), <polynomial constant>)
            uint16_t x_hi = __arm_vgetq_lane_u16(x_, 1)
                    ^ __arm_vgetq_lane_u16(x_, 4)
                    ^ __arm_vgetq_lane_u16(x_, 6);
            x_ = __arm_vsetq_lane_u16(x_hi, x_, 4);
            x_ = __arm_vsetq_lane_u16(x_hi, x_, 6);
            uint32x4_t y_ = __arm_vmullbq_poly_p16(x_, c_p_);

            // hi32(_)
            y_ = __arm_vshlq_n_u32(y_, 1);
            crc = __arm_vgetq_lane_u32(y_, 3)
                    ^ __arm_vgetq_lane_u16(__arm_vreinterpretq_u16_u32(y_), 5)
                    ^ __arm_vgetq_lane_u16(__arm_vreinterpretq_u16_u32(y_), 1);
